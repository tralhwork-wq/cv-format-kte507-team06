...
function App() {
  ...
  const APPS_SCRIPT_ENDPOINT = 'https://script.google.com/macros/s/AKfycbwah0MwOpMsJN34nRmqzHb_Yp-WTJ17vuuqwy7Aq1OEjZmKZV3psnvoSF2p2Umv2tn9/exec';
  const saveToSheet = async () => {
    if (!candidate.name.trim()) { notify('Vui lòng nhập họ tên ứng viên trước khi lưu.', 'error'); return; }
    if (!candidate.phone.trim()) { notify('Vui lòng nhập số điện thoại ứng viên trước khi lưu.', 'error'); return; }
    if (!previewRef.current) { notify('Không tìm thấy bản xem trước CV.', 'error'); return; }
    setSavingSheet(true); setStatus('');
    const stage = document.createElement('div');
    stage.style.position = 'fixed';
    stage.style.left = '-10000px';
    stage.style.top = '0';
    ...
    try {
      ...
      stage.appendChild(clone);
      document.body.appendChild(stage);
      const canvas = await html2canvas(clone, { scale: 3, backgroundColor: '#ffffff', useCORS: true, width: 794, height: 1123, windowWidth: 794, windowHeight: 1123 });
      const base64 = canvas.toDataURL('image/png').replace(/^data:image\/png;base64,/, '');
      const response = await fetch(APPS_SCRIPT_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify({ fullName: candidate.name, phone: candidate.phone, imageBase64: base64 }) });
      const text = await response.text();
      let result: { success?: boolean; error?: string; message?: string };
      try { result = JSON.parse(text); } catch { result = { success: response.ok, message: text }; }
      if (result.success) { notify(result.message || 'Đã lưu CV vào Drive và cập nhật Sheet'); }
      else { notify(result.error || result.message || 'Không thể lưu hồ sơ. Vui lòng thử lại.', 'error'); }
    } catch { notify('Không thể lưu hồ sơ. Vui lòng kiểm tra kết nối và thử lại.', 'error'); }
    finally { if (stage.parentNode) document.body.removeChild(stage); setSavingSheet(false); }
  };
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><main className="editor-area">
      ...
      <section className="candidate-panel"><div className="section-heading"><div><span className="heading-bar" /><h2>Thông tin ứng viên</h2></div><div className="heading-actions"><button className="sheet-button" onClick={saveToSheet} disabled={savingSheet}>{savingSheet ? 'Đang lưu hồ sơ...' : 'Lưu hồ sơ'}</button><button className="outline-button" onClick={refreshPreview}><Zap size={14} /> Tạo bản xem trước</button></div></div>
